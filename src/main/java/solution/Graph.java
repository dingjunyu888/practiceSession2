...
public class Graph {
    ...
    public ArrayList<Integer> noIncomingEdges() {
        ...
        for (int v = 0; v < graph.length; v++) {
            Edge tmp = graph[v];
            while (tmp != null) {
                hasIncoming[tmp.neighbor] = true;
                tmp = tmp.next;
            }
        }
        // Go through the hasIncoming array, and add those to the resulting list for which hasIncoming is false
        for (int k = 0; k < hasIncoming.length; k++) {
            if (hasIncoming[k] == false) // vertex that has no incoming edges
                verticesNoIncomingEdges.add(k);
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(g.noIncomingEdges()); //1, 3
    }
}
